...
#df = session.sql("select * from TRANSPORTATION_HUB.HUB.ROUTES")
#data = df.to_pandas()
#st.write("**Routes**")
#st.dataframe(data, use_container_width=True)

# Get the Vehicle Stop Time data
avg_delay_qry = "select LOCAL_DATE,ROUTE_ID,ROUTE_TYPE,ROUTE_DESC,VEHICLE_ID,VEHICLE_TYPE,STOP_LOCATION_ID \
...
#vehicle_select_df = session.sql("select ROUTE_ID, VEHICLE_ID from TRANSPORTATION_HUB.HUB.VESSEL_STOP_TIME_POSITIONS group by ROUTE_ID, VEHICLE_ID order by ROUTE_ID, VEHICLE_ID")
#vdfpd = vehicle_select_df.to_pandas()
